...
import pytest
import requests
from portfoliomanager.data_client import (
    fetch_equity_details,
    fetch_historical_prices,
    fetch_spy_prices,
)
from portfoliomanager.exceptions import PriceDataUnavailableError
...
def test_fetch_historical_prices_returns_expected_columns() -> None:
    ...
    with patch("portfoliomanager.data_client.requests.get", return_value=mock_response):
...
def test_fetch_historical_prices_sends_correct_query_params() -> None:
    ...
    with patch(
        "portfoliomanager.data_client.requests.get", return_value=mock_response
    ) as mock_get:
        fetch_historical_prices(
...
def test_fetch_historical_prices_raises_on_http_error() -> None:
    ...
    with (
        patch("portfoliomanager.data_client.requests.get", return_value=mock_response),
...
def test_fetch_historical_prices_raises_on_network_error() -> None:
    with (
        patch(
            "portfoliomanager.data_client.requests.get",
