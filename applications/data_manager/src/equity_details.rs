...
/// Boundary morphism: converts an untrusted CSV string from S3 into a validated
/// `Vec<EquityDetail>`. Rows with invalid ticker symbols are discarded with a warning;
/// structural errors (missing columns, wrong field count) return an error.
...
mod tests {
    ...
    fn test_parse_equity_details_csv_valid() {
        ...
        assert_eq!(details[1].ticker().as_str(), "GOOGL");
    }
    ...
    fn test_parse_equity_details_csv_whitespace_trimming() {
        ...
        assert_eq!(details[0].ticker().as_str(), "ECC");
        assert_eq!(details[0].sector(), "TECHNOLOGY");
        assert_eq!(details[0].industry(), "CONSUMER ELECTRONICS");
    }
