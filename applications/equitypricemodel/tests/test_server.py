...
def test_resolve_artifact_key_uses_latest_by_default() -> None:
    mock_s3 = MagicMock()
    mock_ssm = MagicMock()
    error_response = {"Error": {"Code": "ParameterNotFound", "Message": ""}}
    ...
    with (
        ...
        patch(
            "equitypricemodel.server.find_latest_artifact_key"
        ) as mock_find,
    ):
        mock_boto3.client.return_value = mock_ssm
        mock_find.return_value = (
            "artifacts/model-2026/output/model.tar.gz"
        )
...
def test_resolve_artifact_key_uses_ssm_version() -> None:
    ...
    expected = (
        "artifacts/equitypricemodel-trainer-2026-01-15"
        "/output/model.tar.gz"
    )
