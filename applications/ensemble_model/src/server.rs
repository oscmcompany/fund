...
async fn predictions(State(state): State<AppState>) -> impl IntoResponse {
    ...
    if let Some(prediction_array) = predictions.as_array() {
        ...
        state
            .metrics
            .set_row_count(prediction_array.len() as u64);
    }
    ...
    if let Some(pool) = &state.pool {
        ...
        if let Some(prediction_array) = predictions.as_array() {
            match database::insert_predictions(pool, prediction_array, correlation_id, &model_run_id)
                .await
            {
                Ok(rows) => {
...
mod tests {
    ...
    fn make_test_state() -> AppState {
        AppState {
            ...
            metrics: std::sync::Arc::new(crate::state::Metrics::new()),
