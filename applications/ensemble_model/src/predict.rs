...
pub fn consolidate_data(
    ...
    let details = equity_details
        ...
        .select([
            col("ticker"),
            col("sector").cast(DataType::String).str().strip_chars(lit(" ")),
            col("industry").cast(DataType::String).str().strip_chars(lit(" ")),
        ])
        .collect()
        .map_err(|e| PredictionError::DataConsolidation(e.to_string()))?;
...
mod tests {
    ...
    fn test_filter_equity_bars_above_thresholds() {
        let data = DataFrame::new(vec![
            ...
            Column::new("volume".into(), vec![2_000_000i64, 3_000_000, 5_000_000, 4_000_000]),
        ])
        .unwrap();
    ...
    fn test_filter_equity_bars_below_close_threshold() {
        ...
        let tickers: Vec<&str> = result.column("ticker").unwrap().str().unwrap().into_no_null_iter().collect();
