...
pub async fn resolve_artifact_key(
    ...
    let latest = folders
        .last()
        .ok_or(ArtifactError::NoArtifacts)?;
...
pub async fn download_and_load_model(
    ...
    info!(bucket = bucket, key = key, "Downloading model artifact from S3");
...
fn load_model_from_directory(dir: &Path, artifact_key: &str) -> Result<ModelState, ArtifactError> {
    ...
    let parameters = tide_model::config::ModelParameters::load(&parameters_path)
    ...
    let (scaler, continuous_columns, categorical_columns, static_categorical_columns) =
        tide_model::data::Scaler::load(&scaler_path)
            .map_err(|e| ArtifactError::ModelLoad(e.to_string()))?;

    let mappings_path = dir.join("tide_data_mappings.json");
    let mappings_content =
        std::fs::read_to_string(&mappings_path).map_err(|e| ArtifactError::ModelLoad(e.to_string()))?;
    let mappings: tide_model::data::FeatureMappings =
        serde_json::from_str(&mappings_content).map_err(|e| ArtifactError::ModelLoad(e.to_string()))?;

    let num_quantiles = parameters.quantiles.len();
    let model = tide_model::model::TideModel::load(
        dir,
