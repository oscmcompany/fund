use crate::data_manager::data::{
    create_equity_bar_dataframe, create_equity_details_dataframe, EquityBar,
};
...
/// Builds the Hive-partitioned S3 key for a day's equity bars, matching the
/// layout written by the nightly `export_equity_bars()` pg_parquet function:
/// `data/equity/bars/year=YYYY/month=MM/day=DD/data.parquet` (zero-padded).
fn equity_bars_s3_key(date: NaiveDate) -> String {
    format!(
...
/// Writes a day's equity bars to S3 as Hive-partitioned Parquet, overwriting any
/// existing object for that date (partition-level upsert). The DataFrame schema
/// is produced by [`create_equity_bar_dataframe`], which matches the
/// `equity_bars_schema` pandera contract consumed by model training
/// (Int64 millisecond `timestamp`, uppercased `ticker`).
pub async fn write_equity_bars_to_s3(
    state: &State,
    date: NaiveDate,
    bars: &[EquityBar],
) -> Result<(), Error> {
    ...
    let key = equity_bars_s3_key(date);
    info!(
        "Writing {} equity bars to s3://{}/{}",
        bars.len(),
        state.bucket_name,
        key
    );
    ...
    state
        ...
        .send()
        .await
        .map_err(|e| Error::Other(format!("Failed to put object to S3: {}", e)))?;
