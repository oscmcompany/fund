...
impl std::fmt::Display for AlpacaError {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            AlpacaError::Request(error) => write!(formatter, "Request error: {error}"),
            AlpacaError::Api { status, body } => {
                write!(formatter, "API error {status}: {body}")
            }
            AlpacaError::Parse(message) => write!(formatter, "Parse error: {message}"),
        }
    }
}
...
impl AlpacaTradingClient {
    /// Constructs an `AlpacaTradingClient`.
    ///
    ...
    pub async fn submit_long_order(
        ...
        let order: OrderResponse = response.json().await.map_err(|error| {
            AlpacaError::Parse(format!("Failed to parse order response: {error}"))
        })?;
    ...
    pub async fn get_order(&self, alpaca_order_id: &str) -> Result<OrderFill, AlpacaError> {
    ...
    pub async fn close_position(&self, ticker: &str) -> Result<bool, AlpacaError> {
        let url = format!(
    ...
    pub async fn is_market_open(&self) -> Result<bool, AlpacaError> {
        ...
        let clock: ClockResponse = response.json().await.map_err(|error| {
            AlpacaError::Parse(format!("Failed to parse clock response: {error}"))
        })?;
...
mod tests {
    ...
    fn test_alpaca_error_display_parse() {
        let error = AlpacaError::Parse("unexpected field".to_string());
    ...
    fn test_new_uses_paper_url_when_paper_is_true() {
        let client = AlpacaTradingClient::new(make_credentials(), true);
        assert!(client.base_url.contains("paper-api"));
    }

    #[test]
    fn test_new_uses_live_url_when_paper_is_false() {
        let client = AlpacaTradingClient::new(make_credentials(), false);
        assert!(!client.base_url.contains("paper-api"));
