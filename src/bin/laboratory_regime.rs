...
#[derive(Debug, Parser)]
#[command(
    name = "laboratory_regime",
    about = "Asks whether a forecast's per-session reading follows from the state of the market"
)]
struct Arguments {
    #[arg(
...
mod tests {
    ...
    fn parse(values: &[&str]) -> Result<Arguments, clap::Error> {
        Arguments::try_parse_from(
            std::iter::once("laboratory_regime").chain(values.iter().copied()),
        )
    }
