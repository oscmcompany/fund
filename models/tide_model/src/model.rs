...
impl TideModel<NdArray> {
    ...
    pub fn load(
        ...
        match model.clone().load_file(record_path, &burn::record::DefaultFileRecorder::<burn::record::FullPrecisionSettings>::new(), &device) {
            Ok(loaded) => Ok(loaded),
            Err(_) => {
...
impl<B: Backend> TideModel<B> {
    ...
    pub fn new(
        ...
        let feature_projection_1 =
            nn::LinearConfig::new(input_size, hidden_size * 2).init(device);
        let feature_projection_2 =
            nn::LinearConfig::new(hidden_size * 2, hidden_size).init(device);
...
mod tests {
    ...
    fn test_model_forward_shape() {
        ...
        let input =
            Tensor::<NdArray, 2>::zeros([batch_size, input_size], &device);
