...
def test_send_training_notification_on_completion() -> None:
    ...
    with (
        patch.dict(
            "os.environ",
            {
                "FUND_TRAINING_NOTIFICATION_SENDER_EMAIL": "sender@example.com",
                "FUND_TRAINING_NOTIFICATION_RECIPIENT_EMAILS": "recipient@example.com",
            },
        ),
        patch("tide.notifications.boto3") as mock_boto3,
    ):
...
def test_send_training_notification_skips_when_not_configured() -> None:
    ...
    with (
        patch.dict(
            "os.environ",
            {
                "FUND_TRAINING_NOTIFICATION_SENDER_EMAIL": "",
                "FUND_TRAINING_NOTIFICATION_RECIPIENT_EMAILS": "",
            },
        ),
        patch("tide.notifications.boto3") as mock_boto3,
...
def test_send_training_notification_handles_ses_error() -> None:
    ...
    with (
        ...
        patch("tide.notifications.boto3") as mock_boto3,
        patch("tide.notifications.logger") as mock_logger,
    ):
        mock_boto3.client.return_value = mock_ses
        send_training_notification(flow, flow_run, state)
