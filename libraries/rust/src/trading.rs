...
mod tests {
    ...
    fn test_equity_portfolio_snapshot_end_of_day() {
        ...
        assert_eq!(snapshot.snapshot_type().as_str(), "end_of_day");
        assert!(snapshot.gross_return().is_some());
    }

    #[test]
    fn test_rebalance_session_status_as_str() {
        assert_eq!(RebalanceSessionStatus::Completed.as_str(), "completed");
        assert_eq!(RebalanceSessionStatus::Failed.as_str(), "failed");
    }

    #[test]
    fn test_equity_pair_status_as_str() {
        assert_eq!(EquityPairStatus::Open.as_str(), "open");
        assert_eq!(EquityPairStatus::Closed.as_str(), "closed");
    }

    #[test]
    fn test_allocation_side_as_str() {
        assert_eq!(AllocationSide::Long.as_str(), "LONG");
        assert_eq!(AllocationSide::Short.as_str(), "SHORT");
    }

    #[test]
    fn test_allocation_action_as_str() {
        assert_eq!(AllocationAction::OpenPosition.as_str(), "OPEN_POSITION");
        assert_eq!(AllocationAction::ClosePosition.as_str(), "CLOSE_POSITION");
        assert_eq!(AllocationAction::Unspecified.as_str(), "UNSPECIFIED");
    }

    #[test]
    fn test_snapshot_type_as_str() {
        assert_eq!(SnapshotType::Intraday.as_str(), "intraday");
        assert_eq!(SnapshotType::EndOfDay.as_str(), "end_of_day");
    }

    #[test]
    fn test_rebalance_sessions_new_returns_none_for_empty() {
        assert!(RebalanceSessions::new(vec![]).is_none());
    }

    #[test]
    fn test_equity_pairs_new_returns_none_for_empty() {
        assert!(EquityPairs::new(vec![]).is_none());
    }

    #[test]
    fn test_equity_allocations_new_returns_none_for_empty() {
        assert!(EquityAllocations::new(vec![]).is_none());
    }

    #[test]
    fn test_equity_orders_new_returns_none_for_empty() {
        assert!(EquityOrders::new(vec![]).is_none());
    }

    #[test]
    fn test_portfolio_snapshots_new_returns_none_for_empty() {
